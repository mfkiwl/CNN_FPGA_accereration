...
int main() {
	...
	uint8_t input_image[7056] = { // 84 * 84
#include "image_84_84_x10"
	};
	uint8_t conv_weight[9] = {		// 3 * 3
#include "conv_weight"
	};

	uint8_t conv_activation[6724] = { // 82* 82
#include "conv_activation_fixed"
	};

	uint8_t max_pooling_activation[1681] = {	// 41 * 41
#include "max_pooling_activation_fixed"
	...
	for (i=0; i<1681; i++)
	{
		...
		if ( act == 0 )
			lower_bound = act; // lower_bound = 0
		else
			lower_bound = act -1; // acr has vale => lb = act-1

		if ( activation_fpga[i] & 0x80 != 0 ) // sign check => if negative break
			break;
		if ( activation_fpga[i] < lower_bound ) // less than lb of ( max pooling activation ) => break
			break;
